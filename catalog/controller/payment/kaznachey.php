...
class ControllerPaymentkaznachey extends Controller {
  ...
  public function pay() 
  {
	...
	$kaznachey_currency_to_uah  = $this->config->get('kaznachey_currency_to_uah');
	...
	foreach ($getProducts as $key=>$product) { 
		...
		$products[$i]['ProductPrice'] = number_format(round($product['price'] * $kaznachey_currency_to_uah , 2), 2, '.', '');
		$products[$i]['ProductId'] = $product['model'];
		$product_count += $product['quantity'];
		$amount2 += $products[$i]['ProductPrice'] * $product['quantity'];
		$products[$i]['ImageUrl'] = (isset($product['image']))?'http://'.$_SERVER['HTTP_HOST'] .'/image/'. $product['image']:'';
		$i++;
	}
	
	$amount = number_format(round($order_info['total'] * $kaznachey_currency_to_uah , 2), 2, '.', '');
	...
	if($amount != $amount2)
	{
		...
		$products[$i]['ProductPrice'] = number_format(round($tt * $kaznachey_currency_to_uah , 2), 2, '.', '');
	...
	$copy_success_url = 'http://'.$_SERVER['HTTP_HOST']  . '/kaznachey_result.php';
